...
class Processor {
 public:
  ...
  /**
   * @brief transforms the input to some output using the context
   */
  virtual Output process(const model::Context &ctx, const Input &data) const = 0;
};

/**
 * @brief a specialization of a processor that just produces some output without input
 */
template <typename Output>
class Processor<void, Output> {
 public:
  virtual ~Processor() = default;
  /**
   * @brief produces some output using the context
   */
  virtual Output process(const model::Context &ctx) const = 0;
};
...
/**
 * @brief a specialization of a pipeline that is a processor without input
 */
template <typename Output>
class Pipeline<void, Output> : public Processor<void, Output> {
 private:
  virtual void inc_ref_counter() const {}
  virtual bool dec_ref_counter() const {
    return false;
 ...
 public:
  ...
  template <typename NextOutput>
  auto add_next(const Processor<Output, NextOutput> &next) const {
    class ChainedStage : public Pipeline<void, NextOutput>, private tlx::ReferenceCounter {
      friend class tlx::CountingPtr<ChainedStage>;

     private:
      const Pipeline<void, Output> &_current;
      const Processor<Output, NextOutput> &_next;
     ...
     public:
      ...
      ChainedStage(const Pipeline<void, Output> &current, const Processor<Output, NextOutput> &next)
          : _current(current), _next(next) {}
      ...
      NextOutput process(const model::Context &ctx) const override {
        auto result = _current.process(ctx);
        return _next.process(ctx, std::forward<Output>(result));
      }
    };
    inc_ref_counter();
...
template <typename Output>
using Producer = Pipeline<void, Output>;
